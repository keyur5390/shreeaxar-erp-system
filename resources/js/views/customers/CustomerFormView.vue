<script setup lang="ts">
import { computed, reactive, ref, watch } from 'vue'
import { useRoute, useRouter } from 'vue-router'
import { useForm, useFieldArray, ErrorMessage, Field } from 'vee-validate'
import { useMutation, useQuery, useQueryClient } from '@tanstack/vue-query'
import * as yup from 'yup'
import PageHeader from '@/components/ui/PageHeader.vue'
import Popover from '@/components/ui/Popover.vue'
import PopoverTrigger from '@/components/ui/PopoverTrigger.vue'
import PopoverContent from '@/components/ui/PopoverContent.vue'
import { customersService } from '@/services/customers.service'
import { addressTypesService } from '@/services/address-types.service'
import { countriesService } from '@/services/countries.service'
import { mastersService } from '@/services/masters.service'
import { useToast } from '@/composables/useToast'
import { useFormDirtyGuard } from '@/composables/useFormDirtyGuard'
import { useBreakpoint } from '@/composables/useBreakpoint'
import { ValidationError } from '@/services/api'
import type { AddressType, CustomerFormAddress } from '@/types'

const route = useRoute()
const router = useRouter()
const queryClient = useQueryClient()
const { toast } = useToast()
const { isMobile } = useBreakpoint()

const customerId = computed(() => {
  const id = route.params.id
  return typeof id === 'string' && id !== 'new' ? id : undefined
})
const isEdit = computed(() => Boolean(customerId.value))

const submitError = ref('')
const localAddressTypes = ref<AddressType[]>([])
const popoverOpen = reactive<Record<number, boolean>>({})
const newTypeNames = reactive<Record<number, string>>({})
const createTypeErrors = reactive<Record<number, string>>({})

let emailDebounceTimer: ReturnType<typeof setTimeout> | null = null
let tinDebounceTimer: ReturnType<typeof setTimeout> | null = null

async function checkEmailAvailable(email: string): Promise<boolean> {
  return new Promise((resolve) => {
    if (emailDebounceTimer) clearTimeout(emailDebounceTimer)
    emailDebounceTimer = setTimeout(async () => {
      try {
        const result = await customersService.checkEmail(email, customerId.value)
        resolve(result.available)
      } catch {
        resolve(true)
      }
    }, 400)
  })
}

async function checkTinAvailable(tin: string): Promise<boolean> {
  return new Promise((resolve) => {
    if (tinDebounceTimer) clearTimeout(tinDebounceTimer)
    tinDebounceTimer = setTimeout(async () => {
      try {
        const result = await customersService.checkTin(tin, customerId.value)
        resolve(result.available)
      } catch {
        resolve(true)
      }
    }, 400)
  })
}

const schema = computed(() => yup.object({
  company_name: yup.string().required('Company name is required.').max(300),
  email: yup.string().email('Enter a valid email.').nullable()
    .test('unique-email', 'Email already in use.', async (value) => {
      if (!value) return true
      return checkEmailAvailable(value)
    }),
  secondary_email: yup.string().email('Enter a valid email.').nullable(),
  contact_number: yup.string().max(50).nullable(),
  secondary_contact: yup.string().max(50).nullable(),
  tin_number: yup.string().max(100).nullable()
    .test('unique-tin', 'TIN number already in use.', async (value) => {
      if (!value) return true
      return checkTinAvailable(value)
    }),
  addresses: yup.array().of(
    yup.object({
      address_type_id: yup.string().required('Address type is required.'),
      address_line_1: yup.string().required('Address line 1 is required.').max(500),
      address_line_2: yup.string().max(500).nullable(),
      country_id: yup.string().required('Country is required.'),
      state_id: yup.string().required('State is required.'),
      city: yup.string().max(150).nullable(),
      postal_code: yup.string().max(20).nullable(),
    }),
  ).min(1, 'At least one address is required.').max(10, 'Maximum 10 addresses allowed.'),
}))

const emptyAddress = (): CustomerFormAddress => ({
  address_type_id: '',
  address_line_1: '',
  address_line_2: '',
  country_id: '',
  state_id: '',
  city: '',
  postal_code: '',
})

const { handleSubmit, resetForm, meta, values, setFieldValue: setFieldValueTyped } = useForm({
  validationSchema: schema,
  initialValues: {
    company_name: '',
    email: '',
    secondary_email: '',
    contact_number: '',
    secondary_contact: '',
    tin_number: '',
    addresses: [emptyAddress()],
  },
})

const setFieldValue = setFieldValueTyped as (field: string, value: unknown) => void

const { fields: addressFields, push: pushAddress, remove: removeAddress } = useFieldArray<CustomerFormAddress>('addresses')

const { allowNavigation } = useFormDirtyGuard(computed(() => meta.value.dirty))

const customerQuery = useQuery({
  queryKey: computed(() => ['customers', customerId.value]),
  queryFn: () => customersService.get(customerId.value!),
  enabled: computed(() => isEdit.value),
})

const addressTypesQuery = useQuery({
  queryKey: ['address-types'],
  queryFn: () => addressTypesService.list(),
})

const countriesQuery = useQuery({
  queryKey: ['countries-dropdown'],
  queryFn: () => countriesService.list({ per_page: 200 }),
})

const addressTypes = computed(() => {
  const remote = addressTypesQuery.data.value ?? []
  const merged = [...remote]
  localAddressTypes.value.forEach((type) => {
    if (!merged.some((item) => item.id === type.id)) merged.push(type)
  })
  return merged
})

const countries = computed(() => countriesQuery.data.value?.items ?? [])

watch(
  () => addressTypesQuery.data.value,
  (types) => {
    if (types) localAddressTypes.value = []
  },
)

watch(
  () => customerQuery.data.value,
  (customer) => {
    if (!customer) return
    resetForm({
      values: {
        company_name: customer.company_name,
        email: customer.email ?? '',
        secondary_email: customer.secondary_email ?? '',
        contact_number: customer.contact_number ?? '',
        secondary_contact: customer.secondary_contact ?? '',
        tin_number: customer.tin_number ?? '',
        addresses: customer.addresses?.length
          ? customer.addresses.map((address) => ({
            address_type_id: address.address_type_id ?? '',
            address_line_1: address.address_line_1,
            address_line_2: address.address_line_2 ?? '',
            country_id: address.country_id,
            state_id: address.state_id ?? '',
            city: address.city ?? '',
            postal_code: address.postal_code ?? '',
          }))
          : [emptyAddress()],
      },
    })
  },
  { immediate: true },
)

const statesCache = ref<Record<string, Awaited<ReturnType<typeof mastersService.getStates>>>>({})

async function loadStates(countryId: string) {
  if (!countryId) return []
  if (statesCache.value[countryId]) return statesCache.value[countryId]
  const states = await mastersService.getStates(countryId)
  statesCache.value[countryId] = states
  return states
}

watch(
  () => values.addresses?.map((address) => address.country_id),
  async (countryIds, prevIds) => {
    if (!countryIds) return
    countryIds.forEach(async (countryId, index) => {
      if (countryId && prevIds?.[index] !== countryId) {
        await loadStates(countryId)
        if (prevIds?.[index] && prevIds[index] !== countryId) {
          setFieldValue(`addresses[${index}].state_id`, undefined)
        }
      }
    })
  },
  { deep: true },
)

watch(
  () => values.addresses,
  (addresses) => {
    addresses?.forEach((address) => {
      if (address.country_id && !statesCache.value[address.country_id]) {
        loadStates(address.country_id)
      }
    })
  },
  { immediate: true, deep: true },
)

function getStatesForCountry(countryId: string) {
  return statesCache.value[countryId] ?? []
}

function onCountryChange(index: number, countryId: string) {
  setFieldValue(`addresses[${index}].state_id`, undefined)
  if (countryId) loadStates(countryId)
}

const createTypeMutation = useMutation({
  mutationFn: ({ name }: { name: string; index: number }) => addressTypesService.create({ name }),
  onSuccess: (created, variables) => {
    localAddressTypes.value = [...localAddressTypes.value, created]
    queryClient.invalidateQueries({ queryKey: ['address-types'] })
    setFieldValue(`addresses[${variables.index}].address_type_id`, created.id)
    popoverOpen[variables.index] = false
    newTypeNames[variables.index] = ''
    createTypeErrors[variables.index] = ''
    toast('Address type created successfully.', 'success')
  },
  onError: (error: unknown, variables) => {
    createTypeErrors[variables.index] = error instanceof ValidationError
      ? Object.values(error.errors).flat()[0] ?? error.message
      : (error as { response?: { data?: { message?: string } } })?.response?.data?.message
        ?? (error instanceof Error ? error.message : 'Unable to create address type.')
  },
})

function saveAddressType(index: number) {
  const name = (newTypeNames[index] ?? '').trim()
  if (!name) {
    createTypeErrors[index] = 'Type name is required.'
    return
  }
  createTypeErrors[index] = ''
  createTypeMutation.mutate({ name, index })
}

function getPopoverOpen(index: number): boolean {
  return Boolean(popoverOpen[index])
}

function setPopoverOpen(index: number, value: boolean) {
  popoverOpen[index] = value
}

function getNewTypeName(index: number): string {
  return newTypeNames[index] ?? ''
}

function setNewTypeName(index: number, value: string) {
  newTypeNames[index] = value
}

function onTypeNameEnter(index: number, event: KeyboardEvent) {
  event.preventDefault()
  saveAddressType(index)
}

const saveMutation = useMutation({
  mutationFn: async (formValues: typeof values) => {
    const payload = {
      company_name: formValues.company_name,
      email: formValues.email || null,
      secondary_email: formValues.secondary_email || null,
      contact_number: formValues.contact_number || null,
      secondary_contact: formValues.secondary_contact || null,
      tin_number: formValues.tin_number || null,
      addresses: formValues.addresses?.map((address) => ({
        address_type_id: address.address_type_id,
        address_line_1: address.address_line_1,
        address_line_2: address.address_line_2 || null,
        country_id: address.country_id,
        state_id: address.state_id || null,
        city: address.city || null,
        postal_code: address.postal_code || null,
      })),
    }

    if (isEdit.value && customerId.value) {
      return customersService.update(customerId.value, payload)
    }
    return customersService.create(payload)
  },
  onSuccess: (customer) => {
    allowNavigation()
    toast(isEdit.value ? 'Customer updated successfully.' : 'Customer created successfully.', 'success')
    router.push(`/customers/${customer.id}`)
  },
  onError: (error: unknown) => {
    if (error instanceof ValidationError) {
      submitError.value = Object.values(error.errors).flat()[0] ?? error.message
      return
    }
    submitError.value = (error as { response?: { data?: { message?: string } } })?.response?.data?.message
      ?? (error instanceof Error ? error.message : 'Unable to save customer.')
  },
})

const onSubmit = handleSubmit((formValues) => {
  submitError.value = ''
  saveMutation.mutate(formValues)
})

const pageTitle = computed(() => isEdit.value ? 'Edit Customer' : 'Add New Customer')
const popoverSide = computed(() => (isMobile.value ? 'top' : 'bottom') as 'top' | 'bottom')
</script>

<template>
  <section class="pb-24">
    <PageHeader
      :title="pageTitle"
      subtitle="Manage company details and addresses."
      :breadcrumb="[{ label: 'Customers', href: '/customers' }, { label: pageTitle }]"
    />

    <div v-if="isEdit && customerQuery.isLoading.value" class="rounded-lg border bg-white p-8 text-center text-sm text-slate-500">
      Loading customer…
    </div>

    <form v-else class="space-y-6" @submit="onSubmit">
      <div class="rounded-lg border bg-white p-6 shadow-card">
        <h2 class="mb-4 text-base font-semibold text-slate-900">Company Info</h2>

        <div class="grid gap-4 sm:grid-cols-2">
          <div class="sm:col-span-2">
            <label class="mb-1 block text-sm font-medium text-slate-700">Company Name *</label>
            <Field name="company_name" class="w-full rounded-md border px-3 py-2" />
            <ErrorMessage name="company_name" class="mt-1 block text-xs text-red-600" />
          </div>

          <div>
            <label class="mb-1 block text-sm font-medium text-slate-700">Primary Email</label>
            <Field name="email" type="email" class="w-full rounded-md border px-3 py-2" />
            <ErrorMessage name="email" class="mt-1 block text-xs text-red-600" />
          </div>

          <div>
            <label class="mb-1 block text-sm font-medium text-slate-700">Secondary Email</label>
            <Field name="secondary_email" type="email" class="w-full rounded-md border px-3 py-2" />
            <ErrorMessage name="secondary_email" class="mt-1 block text-xs text-red-600" />
          </div>

          <div>
            <label class="mb-1 block text-sm font-medium text-slate-700">Primary Contact</label>
            <Field name="contact_number" class="w-full rounded-md border px-3 py-2" />
            <ErrorMessage name="contact_number" class="mt-1 block text-xs text-red-600" />
          </div>

          <div>
            <label class="mb-1 block text-sm font-medium text-slate-700">Secondary Contact</label>
            <Field name="secondary_contact" class="w-full rounded-md border px-3 py-2" />
            <ErrorMessage name="secondary_contact" class="mt-1 block text-xs text-red-600" />
          </div>

          <div>
            <label class="mb-1 block text-sm font-medium text-slate-700">TIN Number</label>
            <Field name="tin_number" class="w-full rounded-md border px-3 py-2" />
            <ErrorMessage name="tin_number" class="mt-1 block text-xs text-red-600" />
          </div>
        </div>
      </div>

      <div class="rounded-lg border bg-white p-6 shadow-card">
        <div class="mb-4 flex items-center justify-between">
          <h2 class="text-base font-semibold text-slate-900">Addresses</h2>
          <button
            v-if="addressFields.length < 10"
            type="button"
            class="text-sm font-medium text-brand-blue hover:underline"
            @click="pushAddress(emptyAddress())"
          >
            + Add Address
          </button>
        </div>
        <ErrorMessage name="addresses" class="mb-3 block text-xs text-red-600" />

        <div class="space-y-4">
          <article
            v-for="(field, index) in addressFields"
            :key="field.key"
            class="rounded-lg border border-slate-200 p-4"
          >
            <div class="mb-3 flex items-center justify-between">
              <h3 class="text-sm font-semibold text-slate-800">Address {{ index + 1 }}</h3>
              <button
                type="button"
                class="text-sm text-red-600 disabled:opacity-40"
                :disabled="addressFields.length <= 1"
                @click="removeAddress(index)"
              >
                Remove
              </button>
            </div>

            <div class="grid gap-4 sm:grid-cols-2">
              <div>
                <label class="mb-1 block text-sm font-medium text-slate-700">Address Type *</label>
                <Field :name="`addresses.${index}.address_type_id`" as="select" class="w-full rounded-md border px-3 py-2">
                  <option value="">Select type</option>
                  <option v-for="type in addressTypes" :key="type.id" :value="type.id">{{ type.name }}</option>
                </Field>
                <ErrorMessage :name="`addresses.${index}.address_type_id`" class="mt-1 block text-xs text-red-600" />

                <Popover :open="getPopoverOpen(index)" :side="popoverSide" @update:open="setPopoverOpen(index, $event)">
                  <PopoverTrigger>
                    <span class="mt-1 inline-block text-xs font-medium text-brand-blue hover:underline">
                      + Create New Type
                    </span>
                  </PopoverTrigger>
                  <PopoverContent class="w-64">
                    <div class="space-y-3">
                      <label class="block text-sm font-medium text-slate-700">Type Name</label>
                      <input
                        :value="getNewTypeName(index)"
                        type="text"
                        class="w-full rounded-md border px-3 py-2 text-sm"
                        placeholder="e.g. Billing"
                        @input="setNewTypeName(index, ($event.target as HTMLInputElement).value)"
                        @keydown.enter.prevent="onTypeNameEnter(index, $event)"
                      />
                      <p v-if="createTypeErrors[index]" class="text-xs text-red-600">{{ createTypeErrors[index] }}</p>
                      <div class="flex justify-end gap-2">
                        <button
                          type="button"
                          class="rounded-md border px-3 py-1.5 text-xs"
                          @click="setPopoverOpen(index, false)"
                        >
                          Cancel
                        </button>
                        <button
                          type="button"
                          class="rounded-md bg-brand-blue px-3 py-1.5 text-xs font-semibold text-white disabled:opacity-50"
                          :disabled="createTypeMutation.isPending.value"
                          @click="saveAddressType(index)"
                        >
                          {{ createTypeMutation.isPending.value ? 'Saving…' : 'Save' }}
                        </button>
                      </div>
                    </div>
                  </PopoverContent>
                </Popover>
              </div>

              <div>
                <label class="mb-1 block text-sm font-medium text-slate-700">Country *</label>
                <Field
                  :name="`addresses.${index}.country_id`"
                  as="select"
                  class="w-full rounded-md border px-3 py-2"
                  @change="onCountryChange(index, ($event.target as HTMLSelectElement).value)"
                >
                  <option value="">Select country</option>
                  <option v-for="country in countries" :key="country.id" :value="country.id">{{ country.name }}</option>
                </Field>
                <ErrorMessage :name="`addresses.${index}.country_id`" class="mt-1 block text-xs text-red-600" />
              </div>

              <div class="sm:col-span-2">
                <label class="mb-1 block text-sm font-medium text-slate-700">Address Line 1 *</label>
                <Field :name="`addresses.${index}.address_line_1`" class="w-full rounded-md border px-3 py-2" />
                <ErrorMessage :name="`addresses.${index}.address_line_1`" class="mt-1 block text-xs text-red-600" />
              </div>

              <div class="sm:col-span-2">
                <label class="mb-1 block text-sm font-medium text-slate-700">Address Line 2</label>
                <Field :name="`addresses.${index}.address_line_2`" class="w-full rounded-md border px-3 py-2" />
              </div>

              <div>
                <label class="mb-1 block text-sm font-medium text-slate-700">State *</label>
                <Field :name="`addresses.${index}.state_id`" as="select" class="w-full rounded-md border px-3 py-2">
                  <option value="">Select state</option>
                  <option
                    v-for="state in getStatesForCountry(values.addresses?.[index]?.country_id ?? '')"
                    :key="state.id"
                    :value="state.id"
                  >
                    {{ state.name }}
                  </option>
                </Field>
                <ErrorMessage :name="`addresses.${index}.state_id`" class="mt-1 block text-xs text-red-600" />
              </div>

              <div>
                <label class="mb-1 block text-sm font-medium text-slate-700">City</label>
                <Field :name="`addresses.${index}.city`" class="w-full rounded-md border px-3 py-2" />
              </div>

              <div>
                <label class="mb-1 block text-sm font-medium text-slate-700">Postal Code</label>
                <Field :name="`addresses.${index}.postal_code`" class="w-full rounded-md border px-3 py-2" />
              </div>
            </div>
          </article>
        </div>
      </div>

      <p v-if="submitError" class="text-sm text-red-600">{{ submitError }}</p>

      <div class="sticky bottom-0 z-10 -mx-4 border-t bg-white/95 px-4 py-4 backdrop-blur sm:static sm:mx-0 sm:border-0 sm:bg-transparent sm:px-0 sm:py-0" style="padding-bottom: calc(1rem + env(safe-area-inset-bottom))">
        <div class="flex justify-end gap-3">
          <button
            type="button"
            class="rounded-md border px-4 py-2 text-sm"
            @click="router.push('/customers')"
          >
            Cancel
          </button>
          <button
            type="submit"
            class="rounded-md bg-brand-blue px-4 py-2 text-sm font-semibold text-white disabled:opacity-50"
            :disabled="saveMutation.isPending.value"
          >
            {{ saveMutation.isPending.value ? 'Saving…' : 'Save Customer' }}
          </button>
        </div>
      </div>
    </form>
  </section>
</template>
