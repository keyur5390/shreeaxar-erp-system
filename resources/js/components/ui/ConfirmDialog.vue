...
<template>
  <Teleport to="body">
    <div v-if="open" class="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/50 sm:items-center sm:p-4">
      <section
        role="alertdialog"
      ...
      >
        <div class="overflow-y-auto p-6">
          ...
          <input v-if="requireType" v-model="typed" class="mt-4 w-full rounded-md border px-3 py-2" :placeholder="`Type ${requireType} to continue`" />
        </div>
        <div class="flex justify-end gap-2 border-t p-4" style="padding-bottom: calc(1rem + env(safe-area-inset-bottom))">
          <button class="rounded-md border px-4 py-2" @click="emit('cancel')">Cancel</button>
          <button :disabled="!!requireType && typed !== requireType" :class="['rounded-md px-4 py-2 text-white disabled:opacity-50', confirmVariant === 'destructive' ? 'bg-red-600' : 'bg-brand-blue']" @click="emit('confirm')">{{ confirmLabel }}</button>
        </div>
      </section>
    </div>
  </Teleport>
