...
const props = withDefaults(defineProps<{ modelValue?: File | string | null; maxSizeMB?: number; accept?: string; shape?: 'circle' | 'square'; onRemove?: () => void }>(), { maxSizeMB: 2, accept: 'image/*', shape: 'square' })
...
<template><div class="space-y-2"><label :class="['flex aspect-square w-32 cursor-pointer items-center justify-center overflow-hidden border border-dashed bg-white text-sm text-slate-500', shape === 'circle' ? 'rounded-full' : 'rounded-lg']"><img v-if="preview" :src="preview" class="h-full w-full object-cover" alt="Preview" /><span v-else>Upload</span><input type="file" class="sr-only" :accept="accept" @change="select" /></label><button v-if="preview" type="button" class="text-sm text-red-600" @click="remove">Remove</button><p v-if="error" class="text-sm text-red-600">{{ error }}</p></div></template>
